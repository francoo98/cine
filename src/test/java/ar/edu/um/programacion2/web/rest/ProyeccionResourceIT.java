...
public class ProyeccionResourceIT {
	...
	public static Proyeccion createEntity(EntityManager em) {
		Proyeccion proyeccion = new Proyeccion().fechaInicio(DEFAULT_FECHA_INICIO).fechaFin(DEFAULT_FECHA_FIN)
				.hora(DEFAULT_HORA).estado(DEFAULT_ESTADO);
		return proyeccion;
	}
	...
	public static Proyeccion createUpdatedEntity(EntityManager em) {
		Proyeccion proyeccion = new Proyeccion().fechaInicio(UPDATED_FECHA_INICIO).fechaFin(UPDATED_FECHA_FIN)
				.hora(UPDATED_HORA).estado(UPDATED_ESTADO);
		return proyeccion;
	}
	...
	public void createProyeccion() throws Exception {
		...
		restProyeccionMockMvc.perform(post("/api/proyeccions").contentType(MediaType.APPLICATION_JSON)
				.content(TestUtil.convertObjectToJsonBytes(proyeccion))).andExpect(status().isCreated());
	...
	public void createProyeccionWithExistingId() throws Exception {
		...
		restProyeccionMockMvc.perform(post("/api/proyeccions").contentType(MediaType.APPLICATION_JSON)
				.content(TestUtil.convertObjectToJsonBytes(proyeccion))).andExpect(status().isBadRequest());
	...
	public void getAllProyeccions() throws Exception {
		...
		restProyeccionMockMvc.perform(get("/api/proyeccions?sort=id,desc")).andExpect(status().isOk())
	...
	public void getProyeccion() throws Exception {
		...
		// Get the proyeccion
		restProyeccionMockMvc.perform(get("/api/proyeccions/{id}", proyeccion.getId())).andExpect(status().isOk())
				.andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
	...
	public void getNonExistingProyeccion() throws Exception {
		// Get the proyeccion
		restProyeccionMockMvc.perform(get("/api/proyeccions/{id}", Long.MAX_VALUE)).andExpect(status().isNotFound());
	}
	...
	public void updateProyeccion() throws Exception {
		...
		// Disconnect from session so that the updates on updatedProyeccion are not
		// directly saved in db
		em.detach(updatedProyeccion);
		updatedProyeccion.fechaInicio(UPDATED_FECHA_INICIO).fechaFin(UPDATED_FECHA_FIN).hora(UPDATED_HORA)
				.estado(UPDATED_ESTADO);

		restProyeccionMockMvc.perform(put("/api/proyeccions").contentType(MediaType.APPLICATION_JSON)
				.content(TestUtil.convertObjectToJsonBytes(updatedProyeccion))).andExpect(status().isOk());
	...
	public void deleteProyeccion() throws Exception {
		...
		restProyeccionMockMvc
				.perform(delete("/api/proyeccions/{id}", proyeccion.getId()).accept(MediaType.APPLICATION_JSON))
