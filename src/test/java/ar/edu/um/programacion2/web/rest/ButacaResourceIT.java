...
public class ButacaResourceIT {
	...
	public static Butaca createEntity(EntityManager em) {
		Butaca butaca = new Butaca().fechaDeVenta(DEFAULT_FECHA_DE_VENTA).fila(DEFAULT_FILA).asiento(DEFAULT_ASIENTO);
		return butaca;
	}
	...
	public static Butaca createUpdatedEntity(EntityManager em) {
		Butaca butaca = new Butaca().fechaDeVenta(UPDATED_FECHA_DE_VENTA).fila(UPDATED_FILA).asiento(UPDATED_ASIENTO);
		return butaca;
	}
	...
	public void createButaca() throws Exception {
		...
		// Create the Butaca
		restButacaMockMvc.perform(post("/api/butacas").contentType(MediaType.APPLICATION_JSON)
				.content(TestUtil.convertObjectToJsonBytes(butaca))).andExpect(status().isCreated());
	...
	public void createButacaWithExistingId() throws Exception {
		...
		restButacaMockMvc.perform(post("/api/butacas").contentType(MediaType.APPLICATION_JSON)
				.content(TestUtil.convertObjectToJsonBytes(butaca))).andExpect(status().isBadRequest());
	...
	public void getAllButacas() throws Exception {
		...
		restButacaMockMvc.perform(get("/api/butacas?sort=id,desc")).andExpect(status().isOk())
	...
	public void getButaca() throws Exception {
		...
		// Get the butaca
		restButacaMockMvc.perform(get("/api/butacas/{id}", butaca.getId())).andExpect(status().isOk())
				.andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
	...
	public void getNonExistingButaca() throws Exception {
		// Get the butaca
		restButacaMockMvc.perform(get("/api/butacas/{id}", Long.MAX_VALUE)).andExpect(status().isNotFound());
	}
	...
	public void updateButaca() throws Exception {
		...
		// Disconnect from session so that the updates on updatedButaca are not directly
		// saved in db
		em.detach(updatedButaca);
		updatedButaca.fechaDeVenta(UPDATED_FECHA_DE_VENTA).fila(UPDATED_FILA).asiento(UPDATED_ASIENTO);

		restButacaMockMvc.perform(put("/api/butacas").contentType(MediaType.APPLICATION_JSON)
				.content(TestUtil.convertObjectToJsonBytes(updatedButaca))).andExpect(status().isOk());
	...
	public void deleteButaca() throws Exception {
		...
		// Delete the butaca
		restButacaMockMvc.perform(delete("/api/butacas/{id}", butaca.getId()).accept(MediaType.APPLICATION_JSON))
